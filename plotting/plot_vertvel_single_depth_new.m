...
for mm = 1:length(gs)
    ...
    [v, vf, xvar, yvar, xy_p] = plot_stat2d('xy','v',ii);
    [nx,ny] = size(data);
    figure(75)
    if mm < 4
        ax = subplot(2,3,mm);
    ...
    pcolor(xvar,yvar-24,data'.*(1-vf'))
    hold on
    %streamslice(xvar,yvar-24,u',v',1,'Color',[0 0 0]);
    if mm ~= 1
        plot(cx05, cy05-24,'k')
        plot(cx075,cy075-24,'k')
        %contour(xvar, yvar-24,v',[1 1]*0.01,'r')
        contour(xvar, yvar-24,v',[1 1]*max_val/10,'r')
        contour(xvar, yvar-24,v',[1 1]*max_val/2,'r--')
    else
        st = streamslice(xvar,yvar-24,u',v'+0.883,0.1);
        %st = streamslice(xvar,yvar-24,u',v',0.3);
        plot(0, -3.22, '.w', 'MarkerSize',27)
    end
    plot([1 1]*-1,[-5 5],'k--')
    plot([1 1]* 1,[-5 5],'k--')
    ...
    if mm == 1
        text(gca,-0.45,0.82,subplot_labels(mm),...
            'Color','k','Units','normalized','Interpreter','Latex')
    else
        text(gca,-0.3,0.82,subplot_labels(mm),...
            'Color','k','Units','normalized','Interpreter','Latex')
    end
    ...
    % plot 1D structure
    figure(76)
    w_mid = v(round(nx/2),:).*(1-vf(round(nx/2),:));
    plot(w_mid,yvar-24)
...
cd('../../figures')
print_figure('vertvel_gamm_new','format','jpeg','size',[8 4])
