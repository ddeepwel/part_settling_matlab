...
for mm = 1:length(cases)
    ...
    hold on
    if strcmp(cases{mm}, 'gamm1.0')
        p2(mm) = plot(time(inds), u_drift(inds) / w2_w1(mm) / uh_w2, 'k');
    else
        p2(mm) = plot(time(inds), u_drift(inds) / w2_w1(mm) / uh_w2, 'Color', cols(mm-1,:));
    end
    xlim([0 80])
    ylim([0 2])
    yticks(0:2)
...
cd('../../figures')
fname = sprintf('part_angle_drift_%s',strrep(angle_dist,'.',''));
