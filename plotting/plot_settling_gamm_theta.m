...
for mm = 1:length(cases1)
    ...
    if strcmp(cases1{mm},'gamm1.0')
        p(mm) = plot(t_p0(inds), (v_p0(inds)+v_p1(inds))/2, 'k');
    else
        p(mm) = plot(t_p0(inds), (v_p0(inds)+v_p1(inds))/2, 'Color', cols(mm-1,:));
    end
    ...
    if strcmp(cases1{mm},'gamm1.0')
        plot(time(inds), v_p(inds),'--', 'Color', [0 0 0])
    else
        plot(time(inds), v_p(inds),'--', 'Color', cols(mm-1,:))
    end
end
